---
title: "Analysis of data from Dell (2010), The Persistent Effects of Peru's Mining Mita"
...
# Regress the log equivalent household consumption on relevant variables (see above) for the household at up to 100km from the border
The coefficient estimate for Mita is -0.284, suggesting that belonging to the Mita district is associated with a 28% decrease in log household consumption (for households in <100km from the border)
```{r}
...
# Same as above, but with a 75km cutoff. 
...
```
# Same as above, but with a 50km cutoff. 
...
# create polynomial variables of diatnce to Potosi
```{r}
...
# Estimate effect on household consumption (100km cutoff) using dpot variables (instead of longitude and latitude)
The estimated coefficient of formerly belonging to the Mita district is similar to previous regressions (33% decrease in household consumption), but it is significant (same holds for 75km and 50km cutoffs, not shown).
An hypothesis is the distance to Potosi is a more economically meaningful variable, which captures a larger share of the residual variance of the outcome variable
```{r}
