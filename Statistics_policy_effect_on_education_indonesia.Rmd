---
title: "Statistica analysis of data from Duflo E., 2001, Schooling and Labor Market Consequences of School Construction in Indonesia: Evidence from an Unusual Policy Experiment."
...
Data refers to the INPRES school construction program studied in Duflo (2001) in “Schooling and Labor Market Consequences of School Construction in Indonesia: Evidence from an Unusual Policy Experiment.” 
```{r}
data = read_csv('inpres_data_corrected.csv')
```
education: years of education,
birth_year: year of birth,
birth_region: region of birth, encoded,
log_wage = logarithm of monthly earnings,
num_schools = avergae number of schools,
...
ed_A = mean(data[(data$dummy_age==1)&(data$high_intensity==1), 'education']$education, na.rm=T)
ed_B = mean(data[(data$dummy_age==1)&(data$high_intensity==0), 'education']$education, na.rm=T)
ed_C = ed_A - ed_B
ed_D = mean(data[(data$dummy_age==0)&(data$high_intensity==1), 'education']$education, na.rm=T)
ed_E = mean(data[(data$dummy_age==0)&(data$high_intensity==0), 'education']$education, na.rm=T)
ed_F = ed_D - ed_E
ed_G = ed_A - ed_D
ed_H = ed_B - ed_E
ed_I = ed_G - ed_H
...
The estimated effect of the program on log earnings is 0.001
```{r}
ed_A = mean(data[(data$dummy_age==1)&(data$high_intensity==1), 'log_wage']$log_wage, na.rm=T)
ed_B = mean(data[(data$dummy_age==1)&(data$high_intensity==0), 'log_wage']$log_wage, na.rm=T)
ed_C = ed_A - ed_B
ed_D = mean(data[(data$dummy_age==0)&(data$high_intensity==1), 'log_wage']$log_wage, na.rm=T)
ed_E = mean(data[(data$dummy_age==0)&(data$high_intensity==0), 'log_wage']$log_wage, na.rm=T)
ed_F = ed_D - ed_E
ed_G = ed_A - ed_D
ed_H = ed_B - ed_E
ed_I = ed_G - ed_H
...
Assumptions: program has causal effect on education (see above) + exclusion restriction criterion (i.e. the program is only related to earnings via education)
```{r}
...
ivreg(log_wage~.-dummy_exposed_schools|.-education,data=df_iv)
```
